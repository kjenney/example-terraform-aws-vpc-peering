...
## Python Script to test connectivity between instances

# Find info for all the instances in this stack.
instances = []
tf_instances = subprocess.check_output(["terraform state list | grep aws_instance"], shell=True)
for i in tf_instances.splitlines():
	decoded_instance = i.decode("utf-8")
	instances.append(decoded_instance)
for instance in instances:
	vpc_info = subprocess.check_output(["terraform state show " + instance + " | grep vpc"], shell=True)
	pprint(vpc_info)
...
#         }
#         else if ($pid == 0) {
#           print("- Testing connection between ${source_vpc}.${source_az} ${dest_vpc}.${dest_az}\n")
